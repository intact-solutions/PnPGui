...
void setupLogging(int stderr_log_level) {
  // TODO(rbentley): We should be able to override / increase this with an env
  // variable
  std::vector<std::string> arguments = {"-v", std::to_string(stderr_log_level)};
  std::vector<char*> argv;
  for (const auto& arg : arguments) {
    // TODO(rbentley): Lets fix this cast up
    argv.push_back(const_cast<char*>(arg.data()));
  }
  argv.push_back(nullptr);
  int argc = argv.size() - 1;

  // This enables loguru to catch signals
  loguru::init(argc, argv.data());
  loguru::g_stderr_verbosity = stderr_log_level;
}
...
void DrawPinIcon(const Pin& pin, bool connected, int alpha) {
  ...
  ax::Widgets::Icon(ImVec2(s_PinIconSize, s_PinIconSize), iconType, connected,
                    color, ImColor(32, 32, 32, alpha));
};

const char* Application_GetName() { return "PnPGui"; }
...
void ShowLeftPane(float paneWidth) {
  ...
  int linkCount = ed::GetSelectedLinks(selectedLinks.data(),
  ...
  for (auto& node : s_Nodes) {
    ...
    if (node.SavedState.empty()) {
      ...
      if (ImGui::IsItemActive())
        drawList->AddImage(s_SaveIcon, ImGui::GetItemRectMin(),
    ...
    if (!node.SavedState.empty()) {
      ...
      if (ImGui::IsItemActive())
        drawList->AddImage(s_RestoreIcon, ImGui::GetItemRectMin(),
                           ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1),
                           IM_COL32(255, 255, 255, 96));
      else if (ImGui::IsItemHovered())
        drawList->AddImage(s_RestoreIcon, ImGui::GetItemRectMin(),
...
void Application_Frame() {
  ...
  ed::Begin("Node editor");
  {
    ...
    util::BlueprintNodeBuilder builder(
        s_HeaderBackground, Application_GetTextureWidth(s_HeaderBackground),
    ...
    for (auto& node : s_Nodes) {
      ...
      // const auto ImDrawList_AddRect = [](ImDrawList* drawList, const ImVec2&
      // a, const ImVec2& b, ImU32 col, float rounding, int rounding_corners,
      // float thickness)
      //{
      //    if ((col >> 24) == 0)
      //        return;
  ...
  if (ImGui::BeginPopup("Create New Node")) {
    ...
    if (ImGui::MenuItem("Weird"))
      node = SpawnInputActionNode();
    if (ImGui::MenuItem("Trace by Channel")) node = SpawnInputActionNode();
